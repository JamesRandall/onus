...
| 2026-09-08 | `mandelbrot` | 3 | passed, 2 findings | 2 | 288 s | 8478 | with item 207 the audit files what the row above could only describe: `main` is green but different — callees `std.list.get`, `std.list.len` added and `std.int.to_text` removed; constants `"P2\n800 600\n255\n"`, `"mandelbrot.pgm"`, 800, 600, 255 removed and `"#"`, `"+"`, `"mandelbrot.txt"`, 80, 40, 100 added — proposed as `add_example`; and `escape_count`, refactored, is flagged for one literal `1` the new body no longer writes, which is the noise a constants comparison carries and a reviewer sees through at a glance |

What the readings say so far: the instrument runs end to end and records honestly, including when it cannot run; a subprocess model with a file system is not a reading at all until the tree it sees is the elided one (item 205) and it can read nothing else (item 206) — the same caveat applies to the Claude Code row in the table below, made before both; with those in place, a capable model reconstructs a contract-shaped module green, and the reading that matters is the one v0 cannot make: `main` came back as a different program with a green ledger, because a body's purpose that no contract, example or comment states is exactly the tacit knowledge the audit exists to find, and the finding it should file is "green but different", which item 207 files over callees, effects and literal constants (the last a noisy signal on a refactor, as the `escape_count` row shows, and a decisive one on `main`); a contract-shaped module reconstructs whole; a query does not, and will not until an example pins it against a fake database. The next runs are reporting, checkout and `toml` under the stronger model, and a look at whether the regenerated bodies differ from the originals in ways the interface cannot see (loop spec §8's third finding, not reported in v0).
