...
Recursion: a recursive function declares `decreases` on an expression over its parameters, as a contract clause after `requires`/`ensures` (`decreases n`), checked at every recursive call site. <!-- changed: M3, docs/CHANGES.md item 43 --> <!-- changed: 2026-09-05, docs/CHANGES.md items 120–121 — the call-site and entry obligations are generated, discharged and otherwise checked at runtime; "same expression up to renaming" is parameters numbered by position --> A measure may also be a value of a record, union or list type, meaning the structural order: at each recursive call the argument must be a proper part of the value at entry, a field or a list element reached by pattern matching, field access or `List.get`. The compiler proves this from how the argument was obtained or rejects the function (`E0344 structural measure not shown to decrease`); a structural measure is never checked at runtime. <!-- changed: 2026-09-05, docs/CHANGES.md item 133 — structural measures, needed by every walk over a syntax tree in the compiler in Onus --> A call may also pass the measure on unchanged to another function of the cycle, as to a helper that takes the same node and recurses on its parts; this is accepted when the calls that pass the measure unchanged form no cycle of their own, so that every cycle takes a proper part somewhere, and is `E0344` otherwise. <!-- changed: 2026-09-05, docs/CHANGES.md item 139 — calls passing a structural measure on unchanged, which the type checker in Onus needs --> Mutual recursion requires the same expression (up to renaming) on every function in the cycle; the compiler computes the cycle and reports `E0320 recursive cycle without shared measure` otherwise.
...
<!-- changed: M10, docs/CHANGES.md item 86 — v0 renders the path, interface, ledger, diff and diagnostics/counterexample views from the reports as one static page (`onus review`); promotion, the path condition in the counterexample view, and decisions flowing back as tasks are not implemented --> <!-- changed: 2026-09-08, docs/CHANGE-LOG-04.md, docs/CHANGES.md item 210 — the tool is an Onus program, `self/workbench.onus`, written against `std.view` (§22): its state is the review bundle plus what the reviewer has chosen (the view shown, the ledger filter, the bodies opened), its views are pure functions of that state, and its messages are the closed set of things the page can do. `onus review` pre-renders the page through `std.html` from the same view function and writes the program beside it, so the page reads without scripts and, in a browser, runs live over the same tree. It remains a draft-zone module until its audit is clean; promotion and decisions flowing back as tasks are still not implemented -->
...
- HTTP: `http.request` (and `http.post`, `http.get`) over `io.Net` — one request, one response of status and body; any status is a response, an unreachable host is `NotFound` and a request that does not complete within the timeout is `Other`. The JavaScript runtime fetches in a child process; the native runtime runs `curl`, which must be on the path. <!-- changed: 2026-09-07, docs/CHANGES.md item 196 — the regeneration loop in Onus needs the model providers -->
...
- Clock: `io.now`, a monotonic reading since the program started as a `Duration`, at the host's resolution; not a calendar time. <!-- changed: 2026-09-06, docs/CHANGES.md item 183 — the first M15.6 change; a monotonic reading fits an `Int` exactly on every target, which nanoseconds since the epoch would not on JavaScript (item 99) --> `io.time`, the calendar time as whole milliseconds since 1970-01-01T00:00:00 UTC, an `Int` that is exact on every target; for a record, not for timing. <!-- changed: 2026-09-06, docs/CHANGES.md item 190 — `onus test --assumptions` dates its ledger records -->
- Panic: raise with an obligation id and optional model; `recover` boundary.
- Capabilities: opaque handles for `io.Files`, `io.Env`, `io.Net`, `io.Clock`, `io.Rand`, `io.Console`, `io.Process`, `sql.Db`, plus the `__fake` constructor available only to test modules. `io.run` starts the program through the platform's process creation with both streams captured and a timeout; a program that cannot start is `NotFound`, one that overruns is `Other`, and WebAssembly has no processes, so there every `run` is `Other`. `io.exec` starts a program on the process's own standard streams, inherited, and returns its exit status. <!-- changed: 2026-09-06, docs/CHANGES.md item 184 --> `io.mkdir` creates missing parents; `io.remove_all` removes a file or a directory tree and is `Ok` when nothing is there; `io.list_dir` is a directory's names in code point order. <!-- changed: 2026-09-06, docs/CHANGES.md item 188 --> <!-- changed: 2026-09-06, docs/CHANGES.md item 175 — both natively -->
