...
- *M15.7 — Retire the oracle.* The native fixed-point build of `self/` becomes `bootstrap/`, the stage0 of every later change; a fixture runner in Onus replaces vitest over the TypeScript library for the fixture suite; the TypeScript compiler, frozen since the M15.4 fixed point and kept in the tree with its differential tests until M15.6 has carried the process end to end on the native compiler (decided 2026-09-06), is removed. From here a language change is made in `self/` only. Progress: the fixture runner landed 2026-09-07 (item 198; every existing expectation passes under it unchanged, and the chain builds it into `bootstrap/run_fixtures.js`); the bespoke vitest cases, the scripts and the removal follow. Accept: the fixture suite and the three examples pass under a compiler built from `bootstrap/` without invoking the TypeScript compiler; a deliberate change to an emitter in `self/` is caught by the stage comparison before promotion.

## 10. Testing conventions

- Fixtures are Onus source files with an adjacent `.expect.json` listing expected diagnostic codes (with spans) or expected obligation statuses. A test runner in `test/harness.ts` diffs actual against expected; no hand-written assertions per fixture. <!-- changed: 2026-09-07, docs/CHANGES.md item 198 — the runner in Onus --> From item 198 the runner is `self/fixtures.onus`, built by the bootstrap chain as `run_fixtures.js` and driven by a `fixtures.json` manifest in each fixture directory (`scripts/fixtures.sh` runs them all): a manifest is one section or a list of `sections`, each of a kind — `diagnostics` (fixtures checked to a pass under a root, their diagnostics and, with `ledger`, their obligations against `.expect.json`; `ok_` fixtures and every fixture under `expect_none` must produce none; the codes under the `covers` prefixes less `except` must all be produced), `canonical` (every fixture is its own canonical form; `messy/` pairs), `lowered` (the entry module's target-neutral form against a golden), `examples` (the examples of each source pass on both targets and the targets agree; `allow_refusal` admits E0800), and `programs` (`main` runs on the listed targets with `args` and must write `stdout` and exit with `status`). `builds` (programs that must build for the listed targets, or be refused natively with the `expect`ed codes), `cli` (command-line cases against the compiler under test: a staged working directory — a copy of `stage_dir` or the `stage` files, `write`, `edit`, `remove`, or an earlier case's directory `in` — then `args` with `$dir`, `$stage`, `$root`, `$now` and a marker-computed `$offset` substituted, and the run's `status`, `stdout`, `stderr`, `files`, `files_exist`, `file_hashes` and `count_files` checked after `normalise` rules such as `dates` and `stage`), `next_sweep` (the token present is legal at every position of every source, and the complete answer of `onus next` at every token of the `pinned` sources), and `script` (a shell script whose output is pinned) — item 199. A section or case that `requires` a tool (`z3`, `clang`, `libpq`, `cli`, `native_cli`, `unicode16`) that is not available is skipped with a notice, and a `programs` section may declare a `probe` program (a database answering, say) that must exit 0 for it to run. `--update` rewrites the expectation files.
