...
- *M15.1 — Front end in Onus.* Lexer, parser and canonical printer; `onus fmt` reimplemented. Accept: byte-identical canonical output to the TypeScript printer on every fixture and example, and identical syntax diagnostics (codes and positions). In progress 2026-09-05: the lexer and the parser are done and agree with the TypeScript front end on every source in the repository, the lexer on both targets (docs/CHANGES.md items 126–132); the printer remains.
