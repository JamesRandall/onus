...
<!-- changed: 2026-09-07, docs/CHANGES.md item 200 — the TypeScript compiler is gone; the layout is the compiler in Onus -->

```
onus/
  package.json            workspace root (pnpm): `pnpm onus` is bootstrap/run_cli.js, `pnpm test` the fixture suite
  self/                   the compiler in Onus (M15): lexer, parser, printer, loader, resolve, types, consteval,
                          effects, contracts, verifier (SMT-LIB, z3 through io.Process, module cache), claims,
                          capabilities, paths, the target-neutral lowering and both emitters (JavaScript, LLVM IR),
                          the reports (interface, path, diagnostics, review page), test/mutate/assumptions,
                          next, the regeneration loop (regen), the command line (cli) and the fixture runner (fixtures);
                          zones (the manifest, the dependency rule and the policy bundles; toml reads the manifest)
    runtime/src/dom.ts    the DOM capability and the view primitive (§22; item 208)
    bundle.onus           the standard library and both runtimes as constants (scripts/bundle.mjs), so the
                          compiler carries them
  bootstrap/              stage0: the JavaScript the compiler emitted for itself at the last fixed point
                          (run_cli.js, run_fixtures.js, the modules, std/, onus-runtime/); never edited by hand
  packages/
    runtime/              the JavaScript runtime generated code imports (src/) and the C runtime of the
                          native target (native/)
    stdlib/               Onus source for std.* (§16), compiled by the compiler
      std/                int, float, text, list, map, grid, bytes, hash, results, option, io, http, sql, …
  test/                   the fixture suite (§10): one directory per area, each with a fixtures.json manifest
    syntax/ checker/ paths/ verify/ roundtrip/ codegen/ examples/ stdlib/ native/ next/ sql/ cli/
  docs/schema/            the JSON schemas of the interface, diagnostic, path documents and (loop/) the task,
                          change and proposal documents
  scripts/                bootstrap.sh (the chain), fixtures.sh (the suite), bundle.mjs, change-review.mjs,
                          bench.mjs (the loop benchmark), brew-formula.sh, unicode-tables.mjs
  examples/
    mandelbrot/
    reporting/
    checkout/
...
The runtime package has its own `tsconfig.json`, `strict: true`, `noUncheckedIndexedAccess: true`, `exactOptionalPropertyTypes: true`.
...
4. **types** → type of every expression, exhaustiveness, `sealed` enforcement, `inout` rules, closure capture rules, const parameter binding. After every module: the instantiation graph — a node per (generic function, type parameter), an edge for each type parameter of the caller occurring in an argument of a call to a generic function, expansive when it occurs under a constructor — and `E0345` for an expansive edge on a cycle (polymorphic recursion, spec §3.6; docs/CHANGES.md item 171).
...
12a. **zones** → every module's zone from the manifest (`onus.toml` at the root; `default` and the most specific pattern win), the dependency rule over every resolved cross-module reference (`E0900`, with the `hardened` items of draft modules as the one exception), the modifier's placement (`E0902`), and the zone policy bundles: `hardened` requires a justification on every `assume` (`E0905`); `critical` requires every public function on a path (`E0903`), applies `verified_assumptions_only` (`E0416`), `forbid { recover }` (`E0413`) and `no_third_party_assumes` with the manifest's exceptions (`E0415`) to every path whose entry it holds, and requires every `checked` obligation to be exercised by the coverage ledger (`E0904`). Records the zone of every module for the reports. <!-- changed: 2026-09-07, docs/CHANGE-LOG-03.md, docs/CHANGES.md item 202 -->
...
Native `Map` and `Dict` (docs/CHANGES.md item 173): one insertion-ordered open-addressing table in `onus.c`, entries in insertion order with tombstones, keys hashed by value — a slot for `Int`, `Duration`, `Bool` and `Unit`, the bytes for `Text` — and a key kind the emitter passes on every keyed operation from the call's type arguments, which the IR carries (`targs` on `call`, §6). `Map.put` copies the table. A key of any other type is E0800.
...
**M10 — Review tool.** Static page (once `packages/review`, since item 193 `onus review` in `self/review.onus`) rendering `interface.json` and `path.json`: the path view (graph, ledger rows), interface view, ledger view, diff view. Reads files; computes nothing beyond layout. Accept: the checkout path renders with the assumed leaf highlighted and the gate region drawn. Added 2026-09-03 (docs/CHANGE-LOG-02.md; done 2026-09-04): assumption freshness shown in the path and ledger views. Redefined by docs/CHANGE-LOG-04.md and done 2026-09-08 (docs/CHANGES.md item 210): the page's logic — the graph layout, every view, the ledger filter, the count of bodies opened — is `self/workbench.onus`, an Onus program against `std.view` in the draft zone; `onus review` pre-renders it through `std.html` and writes the program the compiler carries (built by stage0 into `self/bundle.onus` by `scripts/bundle.mjs`) beside the page with the runtime, and a module script of a dozen lines boots `main` with the bundle. `self/review.onus` keeps the data assembly and the shell. Accept: the pinned pages of `test/cli` regenerate from the view; under a fake document (`test/cli/workbench_dom.mjs`) the live page switches views, filters the ledger and counts an opened body. The hand-written script is gone.
...
- *M15.5 — A native compiler.* The aim: a platform-native `onus`, built by the compiler in Onus from its own source (`onus build self/cli.onus --target native`), that runs with neither TypeScript nor node and compiles any Onus program written with every language feature to date — checking, constant evaluation, verification with z3 through `io.run`, the reports, and both targets: JavaScript for the whole language, native for the §19.1 subset as this milestone extends it. Zones and views (CHANGE-LOG-03, -04) come after it. The delta, measured 2026-09-06 by building `self/cli.onus` for the native target: 481 of the compiler's functions refused, 461 of them for `Dict` or `Map`, 8 for `Text.trim`, `lower`, `len` and `graphemes`, 2 for `blake3_hex`, 1 for equality on a type parameter (`List.index_of`); and five primitives the emitted code calls that the C runtime lacks (`io.run`, `io.mkdir`, `Text.code_points`, `Text.of_code_points`, `Text.contains`). The compiler's source has no closures, function values or interfaces, so the native subset's largest gaps do not arise. Work, in order of size: (1) `Dict` and `Map` in the C runtime — insertion-ordered, keyed by `Int` and `Text`, `Map.put` copying — and the emitter representing them as pointers, which retires the `host.js` claim on `std.map` (done 2026-09-06, docs/CHANGES.md item 173; generic code over a `Dict[K, V]` still waits on the specialisation step); (2) the five primitives, `io.run` over `posix_spawn` with pipes; (3) BLAKE3 in C, from the reference implementation; (4) grapheme segmentation and case mapping in C for `Text.graphemes`, `lower`, `len` and `trim`, since the constant evaluator must implement them faithfully for user programs — a rewrite avoids only the `trim` on z3 and `pg_config` output; (5) `List.index_of`'s equality on a type parameter — resolved by specialisation (item 174: the census fell to 14 functions, all text, hashing or `Bytes`); (6) whatever a second census shows, the first reporting one cause per function. (2), (3) and (4) done 2026-09-06 (docs/CHANGES.md item 175): the census is zero, `onus build self/cli.onus --target native` produces `cli`, and that binary, with node absent, checks mandelbrot with every obligation proved through z3 in 0.7 s and builds it for the native target to a byte-identical image; no stdlib module claims `host.js` any more. The whole-language items done 2026-09-06 (item 178): closures and function values, interfaces, runtime quantifiers, `fake` and `old(...)` compile natively in both emitters, and every fixture with examples that builds for JavaScript now builds natively and agrees; what E0800 still refuses is `Spec` values, a generic function used as a value, and `sql` without libpq. The native stage and the native differentials done 2026-09-06 (item 179): `scripts/bootstrap.sh` builds the compiler natively from stage2 and that executable, with node and TypeScript absent from its path, rebuilds the compiler for both targets to stage2's files and IR; `pnpm --filter compiler test:self-native` runs every differential under `test/self/` against the programs of `self/` built natively. The embedding done 2026-09-06 (item 180): `self/bundle.onus` carries the standard library and both runtimes, the loader and both build drivers fall back to it, `onus --version` exists, and the command line built natively checks, builds and runs a program from a directory holding only its source with node absent from the path; the Windows port is set aside (decided 2026-09-06, no Windows machine to hand; it and its release leg are the milestone's one open item, to be taken up when there is one). The build on GitHub done 2026-09-06 (item 181): `ci.yml` runs every gate on each push, `release.yml` builds and publishes `onus` for Linux x86_64, Linux arm64 and macOS arm64 on a version tag; macOS on Intel is not built (decided 2026-09-06). Accept: `onus build self/cli.onus --target native` produces an executable with no E0800; with node absent from PATH, that executable checks and builds every source in the repository with diagnostics, ledgers, JavaScript and LLVM IR identical to the node-hosted compiler's — the differentials under `test/self/` run against it; `scripts/bootstrap.sh` gains a native stage, the native `onus` building `self/cli.onus` for both targets with outputs equal to the node-hosted stage2's; and mandelbrot built by the native `onus` for the native target renders the same image. **The whole language natively (decided 2026-09-06).** The native target compiles every language feature built so far, not only what the compiler needs, so E0800 is left for what a host genuinely cannot provide. Beyond the census above that means, in the emitter: closures and function values (a closure is a code pointer with its captured environment; a declared function used as a value gets an adapter, as on the JavaScript target), interfaces (dictionary passing, which the lowering already does: a dictionary is a record of function pointers), quantifiers evaluated at runtime as loops, `fake` in test modules, `old(...)` snapshots in checked postconditions (a deep copy by type), equality and `TypeInfo` on values whose type is a type parameter, `Spec` values, and the small gaps (`try` on an `Option` inside a `Result` function without `else`, a row decoder over a non-record); in the C runtime, the JavaScript runtime's surface (115 functions today against a 2,300-line C runtime): the 22 primitives that still claim `host.js` — `Map`, `Text` (`bytes`, `compare`, `graphemes`, `len`, `lower`, `upper`, `starts_with`, `trim`, with Unicode tables), `Bytes`, `hash.blake3_hex`, `typeinfo.fields` and `name` — and `sql` through `libpq` as now. One design decision falls out of equality, `old` and `TypeInfo` on type parameters, since a slot carries no type: either the native lowering monomorphises generic functions per instantiation (the checker records every instantiation; each specialisation then knows its concrete types, and descriptors are constants) or values carry runtime type descriptors. Decided 2026-09-06: monomorphisation (§6.1), for the zero-cost representation and the Rust shape. It costs one language rule, made through the language-change process before the native lowering relies on it (done 2026-09-06, `E0345`, docs/CHANGES.md item 171 — the process's first change, from spec to a promoted `bootstrap/`): polymorphic recursion — a generic function calling itself, directly or through others, at an instantiation that wraps one of its own type parameters — is a checker error on every target, since the set of specialisations would be unbounded (Rust and Go reject it likewise). The fixture is a nested type `Nest[T] = Leaf of T | Node of Nest[Pair[T]]` with `depth[T]` calling `depth[Pair[T]]`. What is lost: nested data types whose type encodes a shape invariant, which Onus states as contracts instead — a perfect binary tree as a regular `Tree[T]` with `perfect` and `depth` predicates proves everything except the postcondition that establishes the shape, because the verifier reasons through contracts and never unfolds a body; a definitional axiom for functions with proved termination would close that (§12 open questions). Accept, in addition: every fixture and example that builds for the JavaScript target builds for the native target, its examples pass on both, and the §19.5 differential over the whole fixture suite reports no E0801. **Release (decided 2026-09-06).** The milestone ends with a released command-line compiler: a native `onus` executable for macOS (arm64; Intel is not built, decided 2026-09-06), Linux (x86_64 and arm64) and Windows (x86_64). That adds: the standard library's sources, the C runtime (`onus.c`, `onus.h`, `onus_sql.c`) and the JavaScript runtime embedded in the executable as generated constants, so that `onus` needs no repository, no `--stdlib` and no `--runtime` — the loader reads `std.*` from the bundle when no `--stdlib` is given, a JavaScript build writes the runtime next to the program it emits and the launcher imports it from there, and a native build writes the C sources for `clang` (the emitted JavaScript changes by that import, so both compilers change under the process); a Windows port of the C runtime (process creation, paths, console and file names in UTF-8) and of anything platform-specific in the emitted IR; `onus --version`; a continuous-integration matrix that builds the compiler on each platform from `bootstrap/` (node and clang are dependencies of the build, not of the released binary) and publishes one archive per platform. On a machine with none of node, TypeScript or the repository, `onus check` and `onus build` work; `onus build --target native` and `onus run --target native` work with `clang` installed; z3 remains optional (§12: absent, every obligation is `checked` with one line on stderr). Windows builds are assumed to use `clang` with the MSVC linker; `wasm` is not part of the release.
- *M15.6 — The rest of the toolchain in Onus.* Begun 2026-09-06 with the clock reading (docs/CHANGES.md item 183), the first change made through the process with the chain's native stage and the native differentials in force; `io.exec` (item 184) and the exit status chosen by `main` (item 185) followed the same day, so the three language changes are made, and the command line took them up the same day (item 187): exit statuses as the TypeScript command line reports them, and `run` on the terminal. `test` for examples, properties and laws with obligation coverage followed (item 189), and a fourth language change it exposed, the calendar time `io.time` for the ledger's records (item 190). `test --assumptions` (item 191) and `test --mutate` (item 192) followed, so `test` is whole in Onus, then `review` (item 193) and `next` (item 195), whose instrumentation of the parser surfaced two lowering bugs on conditions with `inout` calls (item 194). `std.http` (item 196) followed, the primitive the loop's model providers need, and `loop` itself (item 197) closed M15.6 on 2026-09-07: the TypeScript `onus` has no command the one in Onus lacks. `test` (native examples and `--assumptions` and `--mutate` natively; the generated JavaScript tests keep vitest as a dependency of the tests, not of the compiler), `review`'s page renderer, `next`, and `loop`'s entry point; and the three language changes the command-line port exposed — an exit status chosen by `main`, a process primitive that inherits the standard streams, a clock reading — each made through `.claude/skills/language-change/SKILL.md` on the native compiler, which is the process's first exercise there (docs/CHANGES.md item 169). Accept: the TypeScript `onus` has no command the native one lacks; the three changes reached the fixed point through the process.
- *M15.7 — Retire the oracle.* The native fixed-point build of `self/` becomes `bootstrap/`, the stage0 of every later change; a fixture runner in Onus replaces vitest over the TypeScript library for the fixture suite; the TypeScript compiler, frozen since the M15.4 fixed point and kept in the tree with its differential tests until M15.6 has carried the process end to end on the native compiler (decided 2026-09-06), is removed. From here a language change is made in `self/` only. Done 2026-09-07 in three changes: the fixture runner (item 198), the bespoke cases as fixtures (item 199), the removal with the scripts and CI on `bootstrap/` (item 200). Accept: the fixture suite and the three examples pass under a compiler built from `bootstrap/` without invoking the TypeScript compiler; a deliberate change to an emitter in `self/` is caught by the stage comparison before promotion.

**M16 — Zones** (docs/CHANGE-LOG-03.md; done 2026-09-07; the regeneration half of promotion runs when `onus zone promote` is given a model and is recorded, not a gate, item 204). Manifest (`onus.toml`), the dependency rule, policy bundles, the `hardened` modifier, zone fields in the reports, `onus zone` commands (static half: policies and the promotion record; the body-regeneration half runs through the loop when a model is given). Made through `.claude/skills/language-change/SKILL.md` in three changes: the `hardened` modifier and the spec text (docs/CHANGES.md item 201), the manifest with the zones pass, `E0900`–`E0902` and the report fields (item 202), the zone commands, the promotions ledger with conditional guarantees, the loop's zone behaviour and the review page's regions (item 203). Accept: the checkout example zoned — `checkout` and `app.*` critical, `vendor.payments` draft with `charge` hardened and depended on by `checkout` — checks; a dependency on a non-hardened draft item fails `E0900`; demoting the reporting example's `app.config` marks the reporting path conditional. The change log's acceptance names `app.core.*`, `app.reporting` and `app.payments`; the worked examples of spec §18 keep their module names, and the zones are those.

**M17 — Views** (docs/CHANGE-LOG-04.md; begun 2026-09-08). `dom` capability (`std/dom.onus`: `Root`, the effects `dom.read`, `dom.write`, `dom.event`, `subtree`), `dom.patch` and `run` in the JavaScript runtime (`dom.ts`, against whatever `globalThis.document` is: a browser's or a fake for tests), `std/view.onus` (`View`, `Attr`, the closed vocabularies, the predicates, the accessibility conditions and `claim accessible`), `dom.Root` as a root capability of `main` on the JavaScript target and `E0800` natively. Made through `.claude/skills/language-change/SKILL.md` (docs/CHANGES.md item 208). Accept: the counter example (`examples/counter`) runs from the JavaScript target against a fake document under node and, under a browser, the real one; its properties (`buttons_have_handlers`, `accessible` over its view) hold; a path from a view function may `forbid { dom.read, dom.write, dom.event }` and a view function that patches the document itself is rejected with `E0413`; a `main` taking `dom.Root` built natively fails with `E0800`. The vocabulary was widened for documents and drawings, `std.html.to_html` added for pre-rendering, and the `style` attribute removed, in item 209. The workbench followed as item 210 (M10). Deferred: patterns with field values in `is` tests (the change log's `v is Element(tag: Button, ..)`).

## 10. Testing conventions

- Fixtures are Onus source files with an adjacent `.expect.json` listing expected diagnostic codes (with spans) or expected obligation statuses; no hand-written assertions per fixture. <!-- changed: 2026-09-07, docs/CHANGES.md items 198 and 200 — the runner in Onus, under test/ --> The runner is `self/fixtures.onus`, built by the bootstrap chain as `run_fixtures.js` and driven by a `fixtures.json` manifest in each directory under `test/` (`scripts/fixtures.sh` runs them all): a manifest is one section or a list of `sections`, each of a kind — `diagnostics` (fixtures checked to a pass under a root, their diagnostics and, with `ledger`, their obligations against `.expect.json`; `ok_` fixtures and every fixture under `expect_none` must produce none; the codes under the `covers` prefixes less `except` must all be produced), `canonical` (every fixture is its own canonical form; `messy/` pairs), `lowered` (the entry module's target-neutral form against a golden), `examples` (the examples of each source pass on both targets and the targets agree; `allow_refusal` admits E0800), and `programs` (`main` runs on the listed targets with `args` and must write `stdout` and exit with `status`). `builds` (programs that must build for the listed targets, or be refused natively with the `expect`ed codes), `cli` (command-line cases against the compiler under test: a staged working directory — a copy of `stage_dir` or the `stage` files, `write`, `edit`, `remove`, or an earlier case's directory `in` — then `args` with `$dir`, `$stage`, `$root`, `$now` and a marker-computed `$offset` substituted, and the run's `status`, `stdout`, `stderr`, `files`, `files_exist`, `file_hashes` and `count_files` checked after `normalise` rules such as `dates` and `stage`), `next_sweep` (the token present is legal at every position of every source, and the complete answer of `onus next` at every token of the `pinned` sources), and `script` (a shell script whose output is pinned) — item 199. A section or case that `requires` a tool (`z3`, `clang`, `libpq`, `cli`, `native_cli`, `unicode16`) that is not available is skipped with a notice, and a `programs` section may declare a `probe` program (a database answering, say) that must exit 0 for it to run. `--update` rewrites the expectation files.
...
- The canonical printer's round trip is pinned over every canonical fixture and the `messy/` pairs (`roundtrip`); the generated-AST property test went with the TypeScript compiler (item 200).
...
- No `any`, no non-null assertions, no `as` casts in the TypeScript that remains (the runtime).
- Passes are pure over `Context`; no module-level mutable state.
- Errors from user programs are diagnostics; the compiler in Onus has no exceptions: a runtime panic of the compiler is a bug, and `E0999` is reserved for a tool it runs (clang) failing.
- No reflection-style tricks in generated code or runtime; the generated JavaScript should read as if a careful person wrote it.
