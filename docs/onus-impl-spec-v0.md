...
- **Self-contained native backend.** The native target emits LLVM IR as text and spawns `clang` to compile and link it (M11), the GHC and Nim shape. The intended end state is Rust's: code generation in-process through LLVM as a library (or an own backend), with an external process only for the final link, so that the compiler in Onus depends on nothing it did not build. Decided 2026-09-06 as a direction; not scheduled before M15.5.
