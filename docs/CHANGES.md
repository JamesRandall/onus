...
180. **The compiler carries the standard library and both runtimes (impl
    spec M15.5, release).** `self/bundle.onus`, generated by
    `scripts/bundle.mjs` and committed, holds every file of
    `packages/stdlib/std`, the JavaScript runtime's `dist/*.js` and the C
    runtime (`onus.c`, `onus.h`, `onus_lib.c`, `onus_sql.c`,
    `unicode_tables.h`, `blake3/`, its licence) as text constants, with the
    root package's version; a test regenerates it and compares. The loader
    reads `std.*` from the bundle when no `--stdlib` (or `ONUS_STDLIB`) is
    given, under the paths `onus:std/…`, and the prelude is always imported.
    `onus build` with no `--runtime` (or `ONUS_RUNTIME`) writes the
    JavaScript runtime to `<out>/onus-runtime/` and generated modules import
    `./onus-runtime/index.js`, climbing out of their own directories
    (`../onus-runtime/index.js` from `std/`); `--target native` with no
    runtime location writes the C sources to `<out>/native/runtime/` and
    compiles against them. `onus --version` prints the version. So that a
    generated JavaScript program depends on nothing installed, the runtime
    hashes with its own BLAKE3 (`packages/runtime/src/blake3.ts`, checked
    against the reference implementation at every structural boundary in
    `test/stdlib/blake3.test.ts`) and `@noble/hashes` is no longer a
    dependency of the runtime; `pg` remains one for programs that use `sql`,
    loaded only when they do. Found on the way and fixed: the command line
    in Onus rejected a bare entry name (`onus check main.onus`) with E0104,
    since the root it derived was the working directory and the path had no
    directory; a bare name now lives at `.`. The TypeScript compiler is
    unchanged: it resolves its runtime from the workspace, and the
    differentials always pass `--stdlib` and `--runtime`. Fixture:
    `test/self/cli.test.ts` builds the command line natively, removes node
    from the path, and checks, builds for JavaScript, runs under node, builds
    natively and runs mandelbrot from a directory holding only its source,
    the two images equal; `test/self/bundle.test.ts` pins the bundle. Review
    artefacts: `self/bundle.onus` is new (a record, two constants, two
    functions, no obligations); the interface diffs of `self/loader.onus`,
    `self/nativebuild.onus` and `self/cli.onus` are empty but for the `usage`
    text, and `self/build.onus` gains `write_bundled_runtime` (public) and
    `runtime_for`; ledger deltas: `build` +2 obligations, both checked (a
    representation and an overflow obligation on the directory depth), the
