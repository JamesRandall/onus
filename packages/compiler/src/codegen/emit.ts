...
import { stripRefinements, substitute, type ConstValue, type Type, type TypeArg, BOOL } from '../types/type.js';
...
class Emitter {
  ...
  /** Inside a `verify` block: `try ... else` yields the else value as the block's result (§20.2). */
  ...
  run(): EmittedModule {
    ...
    const verifies = this.opts.verify === true ? this.verifyFunctions() : [];
  ...
  /**
   * Every `verify` block of the module as `export function verify$<n>({ params })`
   * returning Bool: assertions that fail return false, `try ... else _: v`
   * returns v, and the end of the body returns true (§20.2).
   */
  private verifyFunctions(): VerifyEmit[] {
    const out: VerifyEmit[] = [];
    const fns: A.FnDecl[] = [];
    for (const item of this.m.module.items) {
    ...
    for (const f of fns) {
      ...
      walk(f.body, (n) => {
        ...
        out.push({ name, key: a.key, claim: this.t.qualifiedName(a.claim), def: this.t.qualifiedName(a.fn), at: `${this.t.qualifiedName(a.fn)}:${pos.line}:${pos.col}`, params });
        const destructure = v.params.length === 0 ? '$args' : `{ ${v.params.map((p) => this.local(p.name.text)).join(', ')} }`;
        this.fn = { inout: [], ret: BOOL, ensures: [], olds: new Set(), def: this.t.def(vd), dicts: new Map(), result: null };
        this.verifying = true;
        this.w.block(`export function ${name}(${destructure}) {`, () => {
          this.bodyWithTry(v.body, () => {
            for (const s of v.body.stmts) {
  ...
  private tryExpr(e: A.Try): Piece {
    ...
    if (this.verifying) return piece(`$rt.${innerOption ? 'unwrapOptionOr' : 'unwrapOr'}(${operand}, (${name}) => ${body})`, CALL);
