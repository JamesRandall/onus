...
cd onus
pnpm install
pnpm -r build      # the runtime; the compiler itself runs from bootstrap/ as is
pnpm test          # the fixture suite (scripts/fixtures.sh)
```
...
- **The regeneration loop** of `docs/onus-loop-v0.md` is `onus loop` (`self/regen.onus`): `onus loop run <task.json>` writes function bodies against the compiler, edits nothing else, and opens a change under `.onus/changes/` that the review page shows. Models are Claude Code (`--model claude-code`), the Anthropic API (`--model anthropic`), OpenRouter (`--model openrouter[:<model>]`) or a scripted answer file. Keys go in `.env.local` at the repository root (`ANTHROPIC_API_KEY`, `OPENROUTER_API_KEY`, optionally `OPENROUTER_MODEL`); `node scripts/bench.mjs mandelbrot <model-spec>... --append docs/BENCHMARK.md` logs a run per model in `docs/BENCHMARK.md`.
- **Milestone 15, the compiler in Onus, is complete but for the Windows port.** The TypeScript compiler is gone (M15.7): every pass, both emitters, the command line, the review page, the regeneration loop and the fixture runner are Onus under `self/`, and `bootstrap/` is the compiler as the JavaScript it emitted for itself at the last fixed point. On the way there the front end (`self/`: lexer, parser, canonical printer and `fmt`) was byte-identical to the TypeScript front end on every source; the checker (loading, resolution, types, constant evaluation, effects), the verifier (contracts, lowering to SMT-LIB, z3 through the `Process` capability, claims, capabilities and paths) and the reports (interface, path and diagnostic documents) agree with the TypeScript compiler on every source: diagnostics, the ledger entry for entry, the documents byte for byte; every walk is proved to terminate. Porting the verifier found a soundness bug in the TypeScript one (a callee's `ensures` about an `inout` parameter was a contradiction, so every obligation in a function that pushed to a builder was proved vacuously) and fixed it in both. Code generation for the JavaScript target is in Onus too: the lowering to the target-neutral form and the JavaScript emitter agree with the TypeScript compiler byte for byte on every source, and `scripts/bootstrap.sh` reaches the fixed point — the compiler compiled by itself is identical, file for file, to itself compiled by the TypeScript compiler. Verifying that port found two more bugs in the TypeScript verifier (SMT sort names colliding across modules, and type-derived facts fanning out through recursive types until the heap ran out), fixed in both. The native emitter is in Onus as well and agrees with the TypeScript one on the LLVM IR of every source, and the command line (`check`, `fmt`, `build`, `run`, `interface`, `path`) is in Onus and agrees with the TypeScript one on a fixed set of invocations; `test`, `review`, `loop` and `next` stay TypeScript for now. The next milestone is a platform-native `onus`: the compiler built by itself for the native target, running with neither TypeScript nor node (impl spec M15.5). Its first half is done: the C runtime carries the whole primitive surface of the specification (`Map`, Unicode 16.0 text, BLAKE3, processes), generic code compiles by specialisation, no standard-library module is JavaScript-only, and `onus build self/cli.onus --target native` yields a native compiler that checks and builds programs, z3 included, with node absent; the native emitter compiles the whole language (closures and function values, interfaces, runtime quantifiers, test doubles, `old(...)`), so every fixture that builds for JavaScript builds natively and agrees; the bootstrap chain ends with a native stage in which the compiler, built natively by itself and run with neither node nor TypeScript, rebuilds itself for both targets to the same outputs, and every differential runs against the native compiler; the compiler carries the standard library and both runtimes, so the native `onus` checks, builds and runs a program from a directory holding only its source, with nothing else installed but `clang` for native output; GitHub Actions run every gate on each push and build and publish `onus` for Linux (x86_64, arm64) and macOS (arm64) on a version tag; what remains of the release is the Windows port of the C runtime. The fixture suite (`test/`, one `fixtures.json` per directory) runs under the compiler in Onus (`scripts/fixtures.sh`), and converting the old test suite to it found and fixed two bugs in the Onus compiler that the differential tests had not. The process for changing the language from here is `.claude/skills/language-change/SKILL.md`.
- **Zones** (`docs/CHANGE-LOG-03.md`, spec §21) are implemented: a manifest `onus.toml` gives every module a zone, the dependency rule and the per-zone policy bundles are checked after paths, `hardened` marks the boundary items of a draft module, and `onus zone promote | demote | show` change zones through a signed manifest and a promotions ledger; the loop's context, escalation and proposals follow the zone. Views (`docs/CHANGE-LOG-04.md`) are next.
...
- `self/` — the compiler in Onus: every pass, both targets, the command line, the review page, the regeneration loop and the fixture runner
- `bootstrap/` — the compiler as the JavaScript it emitted for itself at the last fixed point; `pnpm onus` runs it
...
pnpm onus zone show | promote <module> <zone> [--model <spec>] | demote <module> <zone> [--root <dir>]
...
pnpm onus build <entry.onus> [--out <dir>] [--emit js|ir] [--target js|native]
