...
const bounds = { kind: 'requires', text: '0 <= x and x < w and 0 <= y and y < h', at: 'std.grid', def: 'Grid' };
export function filled(value, width, height) {
    if (width <= 0 || height <= 0)
        throw new Panic({ kind: 'requires', text: 'width > 0 and height > 0', at: 'std.grid', def: 'filled' });
...
export function get(w, h, grid, x, y) {
    ...
    if (x < 0 || x >= grid.width || y < 0 || y >= grid.height)
        throw new Panic(bounds, `(${x}, ${y}) in ${grid.width}x${grid.height}`);
