...
export function request(net, method, url, headers, body, timeout_ms) {
    ...
    if (r.error !== undefined)
        return { tag: 'Err', error: { tag: 'Other', detail: `http: ${r.error.message}` } };
    let parsed;
    try {
    ...
    if (typeof parsed !== 'object' || parsed === null)
        return { tag: 'Err', error: { tag: 'Other', detail: 'http: no response' } };
    if ('error' in parsed) {
        if (parsed.error === 'timeout')
            return { tag: 'Err', error: { tag: 'Other', detail: `the request to ${url} did not complete within ${timeout_ms} ms` } };
