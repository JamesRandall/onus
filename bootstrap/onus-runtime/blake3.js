...
function compress(cv, block, counter, blockLen, flags) {
    ...
    for (let r = 0; r < 7; r++) {
        round(s, m);
        if (r < 6)
            m = Uint32Array.from(MSG_PERMUTATION, (i) => m[i] ?? 0);
    }
    for (let i = 0; i < 8; i++) {
        s[i] = ((s[i] ?? 0) ^ (s[i + 8] ?? 0)) >>> 0;
...
function words(bytes, start, len) {
...
function chunk(bytes, start, len, index, extra) {
...
function parent(left, right, extra) {
...
export function blake3(input) {
    ...
    let root;
    if (chunks === 1)
        root = chunk(input, 0, input.length, 0, ROOT);
    else {
        ...
        for (let t = 0; t < chunks - 1; t++) {
            let cv = Uint32Array.from(chunk(input, t * CHUNK_LEN, CHUNK_LEN, t, 0).subarray(0, 8));
            let total = t + 1;
            while ((total & 1) === 0) {
        ...
        let out = chunk(input, last, input.length - last, chunks - 1, 0);
